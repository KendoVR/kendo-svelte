...
<div class="k-widget k-calendar">
    <div class="k-calendar-header k-hstack">
        <Button
            ...
            class="k-calendar-title k-title"
            >{utils.MONTHS[date.getMonth()]} {date.getFullYear()}</Button
        >
        ...
        <span class="k-calendar-nav k-hstack">
            <Button
                ...
                icon="arrow-60-left"
                on:click="{() => {
                    date = dateMath.addMonths(date, -1);
                    weekOffset = 0;
                }}"
            />
            <span
            ...
            <tbody class="k-calendar-tbody">
                {#each dates as week}
                    <tr class="k-calendar-tr">
                        {#each week as day}
                            <td
                                class:k-state-selected="{dateMath.isEqual(
                                    _selectedDate,
                                    day
                                )}"
                                class="k-calendar-td k-other-month k-weekend"
                            >
                                <CalendarCell
                                    date="{day}"
                                    on:select="{(event) => {
